...
# i/o/e/0/1 are dropped from the id alphabet on purpose: they are the characters
# people confuse on screen (i vs 1, o vs 0) or mishear when an id is read aloud
# across Mandarin and English (e). An id exists to be dictated, so the alphabet
# is trimmed instead of the mistakes being tolerated.
CONNECTION_ID_EXCLUDED = "ioe01"
CONNECTION_ID_ALPHABET = "".join(
    c for c in string.ascii_lowercase + string.digits if c not in CONNECTION_ID_EXCLUDED
)
...
async def get_connection_id_length():
    ...
    Returns the length of connection IDs generated by the server, plus the
    alphabet they are drawn from, so the client can filter input against the
    same rule instead of keeping its own copy of it.
    """
    return api_response(
        HTTPStatus.OK,
...
async def create_session(request: CreateSessionRequest):
    ...
    if requested_id:
        ...
        # is also the KDF input — that trade-off is the caller's to make, but
        # the character set is not: it guards the filesystem path below.
